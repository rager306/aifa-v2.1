...
// Import type for ContentType
import type { ContentType } from '@/config/app-config'
...
vi.mock('@/config/app-config', () => ({
  ...
  getOgImagePath: () => '/og-image.jpg',
  getImagePath: (imageType: string) => `/images/${imageType}.jpg`,
  getImageFormat: (imageType: string) => 'jpeg',
  getLogoPath: () => '/logo.png',
  getLoadingIllustration: (theme: 'dark' | 'light') =>
    theme === 'dark' ? '/loading-dark.png' : '/loading-light.png',
  getErrorIllustration: (errorType: '404' | '500', theme: 'dark' | 'light') =>
    errorType === '404' ? '/not-found.png' : '/error-500.png',
  getHomePageIllustration: (theme: 'dark' | 'light') =>
    theme === 'dark' ? '/homepage-dark.png' : '/homepage-light.png',
  getChatbotIllustration: (theme: 'dark' | 'light') =>
    theme === 'dark' ? '/chatbot-dark.png' : '/chatbot-light.png',
  getAuthorConfig: (): AuthorConfig => ({
    name: 'Test Author',
    email: 'author@test.example.com',
  }),
  getSocialUrls: () => ({
...
describe('constructMetadata', () => {
  beforeEach(() => {
    vi.stubGlobal('process', {
      ...
      env: {
        ...process.env,
        NEXT_PUBLIC_GOOGLE_VERIFICATION: 'test-google-token',
        NEXT_PUBLIC_YANDEX_VERIFICATION: 'yandex-token',
        NEXT_PUBLIC_DEFAULT_AUTHOR_NAME: 'Test Author',
        NEXT_PUBLIC_DEFAULT_AUTHOR_EMAIL: 'author@test.example.com',
      },
    })
  })
