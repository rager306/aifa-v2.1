...
import type { RoadmapItem } from "@/config/pages-config/aifa-roadmap-data"
...
const STATUS_CLASSES: Record<RoadmapItem["status"], string> = {
  completed: "bg-primary/10 text-primary border border-primary/40",
  inProgress: " border ",
}

export function AifaRoadmap({ items }: AifaRoadmapProps) {
  return (
    <section aria-labelledby="aifa-roadmap-heading" className="relative mb-12">
      <div className="mb-8 text-center">
        <h2 id="aifa-roadmap-heading" className="text-2xl font-bold tracking-tight">
          AIFA Roadmap
        </h2>
      </div>
      ...
      <ol className="space-y-10">
        {items.map((item, index) => {
          ...
          return (
            <li key={item.id} className="relative flex gap-4 sm:gap-8">
              ...
              >
                ...
                <p className="mt-2 text-sm text-muted-foreground w-full">{item.description}</p>
